...
앞에서 만든 `my-ml-job`을 쿠버네티스 `Job`으로 제작해 봅시다. `$USERNAME/my-ml-job`은 꼭 본인의 이미지로 변경해주세요. 또한 쿠버네티스는 클러스터 시스템이기 때문에 해당 이미지가 꼭 원격 레지스트리 (ex) https://hub.docker.com)에 저장이 되어있어야 합니다. `<USERNAME>`은 로그인하신 계정명을 적으셔야 합니다.
